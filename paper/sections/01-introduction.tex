...
majority of published work in this area --- and \textbf{0.574} when flows from
...
backbone, random, session-disjoint and temporal splitting agree to within one
seed's standard deviation for every method we ran. What costs 0.19 macro-F1
there is \emph{server}-disjoint splitting: testing only on flows to servers
...
\item A measurement of how much the split protocol determines the reported
      result, on two corpora with opposite structure, with every method on
      identical committed splits (\S\ref{sec:protocols}). The effect is 0.418
      macro-F1 on one corpus and 0.002 on the other, for the same change.
...
\item An evaluation of thirteen methods under strict protocols
      (\S\ref{sec:results}), including the finding that on a
      volume-defined taxonomy four independent neural architectures fall within
      0.05 macro-F1 of each other while gradient-boosted trees on thirteen flow
      statistics sit 0.25 above all of them.
\item Five negative architectural results, each with the test that produced it
      (\S\ref{sec:negative}), including a representation-probing protocol that
      shows an adversarial nuisance-removal component leaving the nuisance as
      decodable as it is in the raw input.
\item An artifact containing every split manifest, every result, and the
      failed experiments (\S\ref{sec:threats}).
\end{enumerate}
