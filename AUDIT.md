...
## 9. Second audit: the regenerated corpora

§1–8 audited the artifact as it stood at `090b4b9`. This section audits the
corpora rebuilt from the raw archives with full provenance. Source:
`results/audit/{fiveg_traffic,cesnet_quic22}/audit_summary.md`, generated by
`make audit`.

Both are audited **separately**. The previous corpus concatenated them, and the
concatenation was itself the shortcut: `protocol == 0` was 99.9% one class
because that value recorded which preparer had run (§3, L3).

### 9.1 What changed in the data

| | v1 (`090b4b9`) | 5G Traffic (v2) | CESNET-QUIC22 (v2) |
| --- | --- | --- | --- |
| Rows | 112,121 (merged) | 164,348 | 201,600 |
| Unit of observation | 10 s window over a whole capture *or* a flow | conversation segment | flow |
| Classes | 6, one with 23 rows | 6, balanced 18k–36k | 18, balanced at 11,200 |
| Provenance | none | capture, server IP, port, timestamp | capture day, client/server IP, port, SNI, AS, timestamp |
| Split protocols available | 2 of 6 | 5 of 6 | 5 of 6 |
| Raw consumed | — | 330 M packet rows, 75 captures | 153 M flow rows, 28 days |

`origin_disjoint` is the one unavailable protocol, and correctly so: a
single-source table has one origin and cannot be split by it.

### 9.2 5G Traffic: the random split is not measuring traffic analysis

Macro-F1, all probes on identical committed splits:

| Probe | Random flow | Session-disjoint | Temporal | Server-disjoint | App-disjoint |
| --- | ---: | ---: | ---: | ---: | ---: |
| Majority class | 0.061 | 0.056 | 0.000 | 0.036 | 0.000 |
| **Server IP / prefix only** | **0.999** | 0.765 | 0.269 | 0.337 | 0.055 |
| **Capture session ID only** | **0.983** | **0.000** | 0.079 | 0.325 | 0.164 |
| Destination port only | 0.814 | 0.598 | 0.153 | 0.684 | 0.000 |
| **AppScanner (2016)** | **0.996** | 0.640 | 0.255 | 0.711 | 0.075 |
| All flow scalars (XGB) | 0.992 | 0.849 | 0.294 | 0.694 | 0.238 |
| First 20 packet sizes | 0.985 | 0.703 | 0.271 | 0.710 | 0.128 |
| k-fingerprinting | 0.974 | 0.604 | 0.265 | 0.713 | 0.103 |

Three readings:

1. **Under a random flow split the benchmark is solved by a single identifier.**
   Server IP alone reaches 0.999 and the capture-session ID alone reaches 0.983.
   Any model number obtained under this protocol is uninterpretable, because a
   classifier that learned nothing but "which capture is this" would score in
   the same range.
2. **The session-disjoint split works, and the audit proves it rather than
   asserting it.** `capture_id_only` falls from 0.983 to **exactly 0.000** —
   the grouping does precisely what it claims. AppScanner falls 0.996 → 0.640.
3. **Temporal is the hardest protocol by a wide margin.** Everything collapses
   below 0.31. These captures span May to October 2022, so a temporal split
   also separates them by application version and by network conditions, which
   is a harder shift than session disjointness.

### 9.3 CESNET-QUIC22: the label's own source explains 97% of it

| Probe | Random flow | Session-disjoint | Temporal | Server-disjoint | App-disjoint |
| --- | ---: | ---: | ---: | ---: | ---: |
| Majority class | 0.006 | 0.006 | 0.006 | 0.001 | 0.000 |
| **SNI string only** | **0.967** | **0.967** | **0.968** | 0.791 | 0.004 |
| Server IP / prefix only | 0.772 | 0.768 | 0.760 | 0.026 | 0.028 |
| Server AS only | 0.160 | 0.162 | 0.160 | 0.089 | 0.030 |
| Capture session ID only | 0.041 | 0.006 | 0.011 | 0.028 | 0.044 |
| **AppScanner (2016)** | 0.768 | 0.772 | 0.762 | **0.566** | 0.057 |
| First 20 packet sizes | 0.785 | 0.790 | 0.780 | 0.600 | 0.109 |
| 5 flow statistics (RF) | 0.414 | 0.415 | 0.405 | 0.297 | 0.029 |
| k-fingerprinting | 0.587 | 0.590 | 0.586 | 0.423 | 0.054 |

Four readings, and the first is the one that has to appear in the paper:

1. **The SNI string alone reaches macro-F1 0.967.** This is expected — CESNET's
   labels are derived from SNI by its publishers — and it is the most important
   honesty result in this repository. The task is *"predict what SNI would have
   said, without seeing SNI"*, which is the right task in a world with Encrypted
   Client Hello, but it means the achievable ceiling is set by how well SNI
   itself categorises traffic. No result on this dataset can be read as
   "matching ground truth"; it is matching a name-based heuristic.
2. **Session-disjoint is indistinguishable from random here** (0.772 vs 0.768
   for AppScanner). Flows on different days really are independent, so grouping
   by day buys nothing. **The split protocol's effect is dataset-dependent, not
   universal** — a finding that qualifies §9.2 rather than reinforcing it, and
   the paper has to say both.
3. **Server-disjoint is the binding protocol for CESNET.** AppScanner drops
   0.772 → 0.566 and SNI-only drops 0.967 → 0.791. Testing on servers never
   seen in training is the realistic deployment condition, and it is the one
   the headline table should use for this dataset.
4. **Destination port and transport protocol are unavailable**, not zero: every
   flow is QUIC to port 443. The audit reports `n/a` rather than a number, so a
   reader cannot mistake "this dataset has no port variation" for "port does
   not help".

### 9.4 Leakage verdicts

Exact duplicates across splits: **zero, on every protocol of both datasets.**
The v1 corpus had 53.

The remaining failures are real and are not going away:

| Failure | Where | Why it stands |
| --- | --- | --- |
| `server_ip_disjoint` | every protocol except server-disjoint | Both datasets contact the same servers throughout. Only the server-disjoint protocol controls it, which is why it is reported. |
| `client_ip_disjoint` | every CESNET protocol | One client appears across days. A client-disjoint protocol is possible (`SRC_IP` is retained) and is the obvious next control; recorded in `paper/THREATS.md` §3. |
| `capture_id_disjoint` | random and server-disjoint | Expected: neither protocol groups on capture. |
| near-duplicates | most protocols | 13–41% of sampled test rows have a training row above cosine 0.999 in standardised AppScanner space. Genuinely distinct short flows are indistinguishable in a 72-dimensional summary. The rate is reported on every protocol and is itself informative: 41% under 5G random-flow against 0.2% under app-disjoint. |

The near-duplicate rate is worth reading as a result rather than as a check. On
5G Traffic it tracks the shortcut numbers almost exactly — 41% under random
flow, 14% under session-disjoint, 0.2% under app-disjoint — which is the
mechanism behind §9.2 stated in a second, independent way.

### 9.5 Consequences for the paper

1. **Headline protocol differs by dataset.** Session-disjoint for 5G Traffic
   (where it demonstrably bites); server-disjoint for CESNET-QUIC22 (where
   session-disjointness does not). Reporting both datasets under one protocol
   name would hide that.
2. **The SNI result must be in the paper, prominently.** A reviewer will ask.
   Better that we asked first.
3. **The random-flow contrast is a stronger result than expected on 5G Traffic
   and a null result on CESNET.** Both go in. A measurement claim that holds on
   one dataset and not the other is a finding about when it holds, and is more
   useful than a claim asserted from one dataset.
4. **`temporal` deserves promotion from a robustness check to a headline
   column.** It is the hardest protocol on 5G Traffic by a factor of two, and it
   is the one that matches how a deployed classifier actually ages.
