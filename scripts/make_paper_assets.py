...
def table_wrapper(
    ...
    """Wrap a tabular body in a float.

    ``wide=True`` emits ``table*``, which spans both columns of the sigconf
    layout. A table with more than about five columns does not fit the 3.3in
    single column: LaTeX sets it at natural width anyway and it runs straight
    over the neighbouring column's text, which is what the deployment-cost
    table (eight columns) was doing.

    ``tabcolsep`` tightens the inter-column padding for a table that misses the
    column width by only a few points, which keeps it single-column and dense
    rather than promoting it to a sparse full-width float.
    """
    env = "table*" if wide else "table"
    padding = [f"\\setlength{{\\tabcolsep}}{{{tabcolsep}}}"] if tabcolsep else []
    return "\n".join(
        [
...
def _savefig(figure: Any, path: Path) -> None:
    """Write a figure as PDF for the paper and PNG for the README.

    Two things are going on here.

    The PDF is what LaTeX includes. Matplotlib stamps ``/CreationDate`` into
    every PDF it writes, so a figure regenerated from unchanged results still
    differs byte-for-byte from the committed one and shows up as a spurious
    diff on each ``make paper``. Suppressing the date makes figure generation
    reproducible in the sense that matters here: identical inputs produce an
    identical file.

    The PNG exists because GitHub does not render a PDF inline, and a README
    that shows the results is worth more than one that describes them. It is
    written to ``docs/figures/`` from the same figure object, so the image a
    reader sees on GitHub cannot drift from the one in the paper -- both are
    regenerated by ``make paper`` from the same results.
    """
    figure.savefig(path, metadata={"CreationDate": None})

    png_dir = Path(__file__).resolve().parent.parent / "docs" / "figures"
    png_dir.mkdir(parents=True, exist_ok=True)
    figure.savefig(png_dir / f"{path.stem}.png", dpi=200, metadata={"Creation Time": None})
